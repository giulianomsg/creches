...
<?php if (!empty($errors)): ?>
    <script>
        ...
        window.__appDeferredToasts.push({
            type: 'danger',
            message: <?= json_encode(!empty($errors['general']) ? $errors['general'] : 'Não foi possível salvar a unidade. Revise os campos destacados e tente novamente.', JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>
        });
    </script>
<?php endif; ?>

<form action="<?= htmlspecialchars($actionUrl) ?>" method="post" class="row g-3 needs-validation" novalidate>
    ...
    <div class="col-12">
        ...
        <input type="text" class="form-control<?= isset($errors['name']) ? ' is-invalid' : '' ?>" id="unit_name" name="name" value="<?= htmlspecialchars($old['name'] ?? ($unit['name'] ?? '')) ?>" required data-validation-message="Informe o nome oficial da unidade.">
    ...
    <div class="col-md-3">
        ...
        <input type="text" class="form-control<?= isset($errors['cep']) ? ' is-invalid' : '' ?>" id="unit_cep" name="cep" value="<?= htmlspecialchars($old['cep'] ?? ($unit['cep'] ?? '')) ?>" data-cep-autocomplete data-cep-logradouro="#unit_endereco" data-cep-bairro="#unit_bairro" required data-validation-message="Informe o CEP da unidade.">
    ...
    <div class="col-md-5">
        ...
        <input type="text" class="form-control<?= isset($errors['endereco']) ? ' is-invalid' : '' ?>" id="unit_endereco" name="endereco" value="<?= htmlspecialchars($old['endereco'] ?? ($unit['endereco'] ?? '')) ?>" required data-validation-message="Informe o logradouro da unidade.">
    ...
    <div class="col-md-2">
        ...
        <input type="text" class="form-control<?= isset($errors['numero']) ? ' is-invalid' : '' ?>" id="unit_numero" name="numero" value="<?= htmlspecialchars($old['numero'] ?? ($unit['numero'] ?? '')) ?>" required data-validation-message="Informe o número do endereço.">
    ...
    <div class="col-md-2">
        ...
        <input type="text" class="form-control<?= isset($errors['bairro']) ? ' is-invalid' : '' ?>" id="unit_bairro" name="bairro" value="<?= htmlspecialchars($old['bairro'] ?? ($unit['bairro'] ?? '')) ?>" required data-validation-message="Informe o bairro da unidade.">
    ...
    <div class="col-md-3">
        ...
        <input type="text" class="form-control<?= isset($errors['macrorregiao']) ? ' is-invalid' : '' ?>" id="unit_macrorregiao" name="macrorregiao" list="macroRegionOptions" value="<?= htmlspecialchars($old['macrorregiao'] ?? ($unit['macrorregiao'] ?? '')) ?>" required data-validation-message="Informe a macrorregião de atendimento.">
    ...
    <div class="col-md-3">
        ...
        <input type="number" min="0" class="form-control<?= isset($errors['capacidade']) ? ' is-invalid' : '' ?>" id="unit_capacidade" name="capacidade" value="<?= htmlspecialchars($old['capacidade'] ?? ($unit['capacidade'] ?? '')) ?>" required data-validation-message="Informe a capacidade de vagas da unidade.">
    ...
    <div class="col-12">
        ...
        <div id="unitLocationMap" class="coordinate-map border rounded" data-lat-input="#unit_latitude" data-lng-input="#unit_longitude" data-address-fields="#unit_endereco,#unit_numero,#unit_bairro,#unit_cep" data-geocode-context="São José do Rio Preto - SP" data-geocode-button="#unitGeocodeButton"></div>
        <div class="form-text">Arraste o marcador para ajustar a referência geográfica da unidade.</div>
    </div>
    <div class="col-12 d-flex justify-content-end">
        <button type="button" class="btn btn-outline-primary" id="unitGeocodeButton">Buscar coordenadas pelo endereço</button>
    </div>
